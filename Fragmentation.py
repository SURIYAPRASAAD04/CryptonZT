...
import oqs
import time
from Encryption import create_folder, aes_decrypt, derive_aes_keys
import traceback
...
def escape_mechanism_reconstruction(data_id, aes_iv, support_data, aes_ciphertext, 
                                  ...
                                  falcon_public_key, dilithium_signature,complete):
    try:
        ...
        with oqs.Signature("Dilithium3") as verifier:
            ...
            if not is_valid:
                ...
                print(f"  NTRU ciphertext length: {len(ntru_ciphertext)}")
                print(f"  Signature length: {len(dilithium_signature)}")
                print(f"  Public key length: {len(dilithium_public_key)}")
                return None
